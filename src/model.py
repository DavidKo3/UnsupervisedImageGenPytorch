...
class E(nn.Module):
    """Content extractor for mnist and faked mnist images."""
    def __init__(self, conv_dim=64, use_labels=True):
...
class G1(nn.Module):
    ...
    def __init__(self, conv_dim=64, use_labels=True):
        ...
        # Decoding 
        self.deconv4 = deconv(conv_dim*2, conv_dim*4, 4,1,0, bn=False)
        self.deconv3 = deconv(conv_dim*4, conv_dim*2, 4)
        self.deconv2 = deconv(conv_dim*2, conv_dim,  4)
        self.deconv1 = deconv(conv_dim, 1,4, bn=True)
        
    def forward(self, x):
        out = F.relu(self.deconv4(x))   # (?, 64, 16, 16)
        #print('Generator deconv4 ', out.size())
        out = F.relu(self.deconv3(out))   # (?, 128, 8, 8)
        # print('Generator deconv3 ', out.size())
        out = F.relu(self.deconv2(out))   # (?, 64, 16, 16)
        # print('Generator deconv2 ', out.size())
        out = F.tanh(self.deconv1(out))   # (?, 1, 32, 32)
        # print("Generator deconv1", out.size())
        return out
